...
async function runPhase26Audit() {
  console.log('===========================================================');
  console.log(' CONSTRUCTIONS PHASE 26 — MARKET INTELLIGENCE & MONITORING AUDIT');
  console.log('===========================================================');

  // [1/9] BASELINE DATASET DEPTH AUDIT
  ...
  console.log('\n[1/9] BASELINE DATASET DEPTH & COUNT INTEGRITY:');
  ...
  // [2/9] NEW PRODUCT SURFACES FILE INTEGRITY AUDIT
  console.log('\n[2/9] NEW PRODUCT SURFACES INTEGRITY AUDIT:');
  const marketPageExists = fs.existsSync(path.join(process.cwd(), 'app/market/page.tsx'));
  const changesPageExists = fs.existsSync(path.join(process.cwd(), 'app/changes/page.tsx'));
  const watchlistPageExists = fs.existsSync(path.join(process.cwd(), 'app/watchlist/page.tsx'));
  const watchlistViewerExists = fs.existsSync(path.join(process.cwd(), 'components/WatchlistViewer.tsx'));

  if (marketPageExists && changesPageExists && watchlistPageExists && watchlistViewerExists) {
    console.log('  ✓ All Phase 26 product surfaces (/market, /changes, /watchlist, WatchlistViewer) present.');
  } else {
    console.error('  ❌ Missing Phase 26 product surfaces!');
  }

  // [3/9] PROVENANCE LEDGER AUDIT
  console.log('\n[3/9] PROVENANCE LEDGER & NON-FABRICATION AUDIT:');
  ...
  // [4/9] COMMERCIAL NEUTRALITY AUDIT
  console.log('\n[4/9] COMMERCIAL NEUTRALITY & PROXY LEAD LANGUAGE AUDIT:');
  ...
  // [5/9] YOUTUBE SOURCE LOCK AUDIT
  console.log('\n[5/9] YOUTUBE OFFICIAL SOURCE LOCK AUDIT:');
  ...
  // [6/9] PROJECT INVESTMENT LABEL RENDERING AUDIT
  console.log('\n[6/9] PROJECT INVESTMENT LABEL RENDERING BUG AUDIT:');
  let rawLabelBugs = 0;
  for (const p of realProjectsDataset) {
  ...
  // [7/9] TELEGRAM SERVER-SIDE CREDENTIAL AUDIT
  console.log('\n[7/9] TELEGRAM SERVER-SIDE CREDENTIAL AUDIT:');
  ...
  const hasWatchlistInSitemap = sitemapContent.includes("'/watchlist'");

  if (hasMarketInSitemap && hasChangesInSitemap && hasWatchlistInSitemap) {
    console.log('  ✓ sitemap.ts dynamically incorporates /market, /changes, and /watchlist.');
  } else {
    console.error('  ❌ Sitemap verification failed for Phase 26 routes!');
  }
  ...
  const allPassed = (
    ...
    hasMarketInSitemap &&
    hasChangesInSitemap &&
    hasWatchlistInSitemap
  );

  if (allPassed) {
    console.log('===========================================================');
    console.log('✅ PHASE 26 MARKET INTELLIGENCE & MONITORING AUDIT PASSED 100%');
    console.log('===========================================================');
  } else {
    console.error('===========================================================');
    console.error('❌ PHASE 26 MARKET INTELLIGENCE & MONITORING AUDIT FAILED ');
    console.error('===========================================================');
    process.exit(1);
  }
}
